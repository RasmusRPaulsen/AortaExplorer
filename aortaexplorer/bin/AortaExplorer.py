...
def main():
    ...
    parser.add_argument("-l", "--logfile", action="store_true", help="Write log file to output folder",
                        default=True)

    parser.add_argument("-fmi", "--forced_min_hu", type=float, help="Force a minimum HU value for lumen segmentation",
                        default=None)
    parser.add_argument("-fma", "--forced_max_hu", type=float, help="Force a maximum HU value for lumen segmentation",
                        default=None)
    parser.add_argument("-lhu", "--low_hu", type=float, help="The lowest possible minimum HU value for lumen segmentation",
                        default=80)
    parser.add_argument("-mhu", "--max_hu", type=float, help="The lowest possible maximum HU value for lumen segmentation",
                        default=80)
    parser.add_argument("-clhu", "--calc_low_hu", type=float, help="The minimum HU value for calcification segmentation",
                        default=80)
    parser.add_argument("-cmhu", "--calc_max_hu", type=float, help="The maximum HU value for calcification segmentation",
                        default=80)
    parser.add_argument("-ts", "--ts_centerline", action="store_true", help="Compute centerline from TotalSegmentator segmentation",
                        default=True)
    parser.add_argument("-ix", "--image-x-size", type=int, help="Visualization image x-side length", default=1920)

    parser.add_argument("-iy", "--image-y-size", type=int, help="Visualization image y-side length", default=1080)
    ...
    aortaexplorer(args.input, args.output, aorta_parms,
                  device=args.device, verbose=args.verbose, quiet=args.quiet, write_log_file=args.logfile)
